...
def alnpdftest(infofile, output, refname, groupinfo):
    ...
    for idx in info.index:
        ...
        delfile= os.path.join(output, info.loc[idx].Note + '_del_aln.txt')
        snpfile= os.path.join(output, info.loc[idx].Note + '_snp_aln.txt')

        catcmd = ' '.join(['cat', delfile, snpfile, '>', alnfile])
        #print(catcmd)
        os.system(catcmd)

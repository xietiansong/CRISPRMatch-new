...
from crisprmatch.CMlib.showprocess import showbarprocess
...
def rate_cal_filter(infofile, groupinfo, refname, output, bamdir):
    ...
    outiofilter_file = os.path.join(output, 'filter_wt_reads_number.txt')
    outiofilter = open(outiofilter_file, 'w')
    print("Sample\tfilter", file=outiofilter)
    fa = Fasta(refname)
    print("start calculation!")
    #print("Sample\tmuation\treplace\tinsertion_only\tdeletion_only\tinsert&deletion", file=outio)
    print("Sample\tmuation\treplace\tinsertion_only\tdeletion_only\tinsert&deletion\tmuation_count\treplace_count\tinsertion_only_count\tdeletion_only_count\tinsert&deletion_count\ttotal_read_count", file=outio)
    ...
    for idx in info.index:
        ...
        elif (re.search("crRNA", info.loc[idx].Note)):  ##5'端延伸10，3'端延伸30
            if strand == '+':
                start = info.loc[idx]['start'] - 10
        ...
        for pileupcolumn_filter in samfile.pileup(gene, max_depth=50000):
            ...
            if start > pileupcolumn_filter.pos >= 0 or pileupcolumn_filter.pos > end:
                for pileupread_filter in pileupcolumn_filter.pileups:
                    ...
                    test.add(pileupread_filter.alignment.query_name)
                    if pileupread_filter.alignment.query_name not in replace_left:
                        ...
                        if not pileupread_filter.is_del and not pileupread_filter.is_refskip:
                            querybase_filter = pileupread_filter.alignment.query_sequence[
                                pileupread_filter.query_position]
        ...
        # print(len(test))
        # print(len(replace_side))
        # print(len(replace_left))
        wt_side_set = replace_side - replace_left
        wt_final_set = wt_side_set & wt_set
        #print(len(wt_set - wt_side_set))
        ...
        #print(len(covage))
        total_read = mtreads | wt_final_set ##去合集
        #total_read = mtreads | wt_set ##去合集
        filter_read = covage - total_read
        print(info.loc[idx].Note, end='\t', file=outio)
        #print(len(mtreads)/len(covage)*100, end='\t', file=outio)
        print(len(mthreads_all)/len(total_read) * 100, end='\t', file=outio)
        print(len(replace)/len(total_read)*100, end='\t', file=outio)
        print(len(insert_only)/len(total_read)*100, end='\t', file=outio)
        print(len(deletion_only)/len(total_read)*100, end='\t', file=outio)
        print(len(insert_deletion)/len(total_read)*100, end='\t', file=outio)

        print(len(mthreads_all), end='\t', file=outio)
        print(len(replace), end='\t', file=outio)
        print(len(insert_only), end='\t', file=outio)
        print(len(deletion_only), end='\t', file=outio)
        print(len(insert_deletion), end='\t', file=outio)
        print(len(total_read), end='\n', file=outio)

        print(info.loc[idx].Note, end='\t', file=outiofilter)
        print(len(filter_read), end='\n', file=outiofilter)
        #print(len(filter_read))
...
def display_filter(groupinfo, output):
    """
    :param groupinfo: a description file of details of each group, example: group_infor.txt
    :param output: folder of final result
    :return:
    ...
    mutfile = os.path.join(output, 'mut_rate.all.txt')
